...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    prompt_segment blue yellow "$fg_bold[yellow]%(!.%{%F{yellow}%}.)$USER@%m$fg_no_bold[yellow]"
  else
    prompt_segment blue yellow "$fg_bold[yellow]%(!.%{%F{yellow}%}.)@$USER$fg_no_bold[yellow]"
  fi
}
...
#«»±˖˗‑‐‒ ━ ✚‐↔←↑↓→↭⇎⇔⋆━◂▸◄►◆☀★☗☊✔✖❮❯⚑⚙
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ $number_modified -gt 0 && $number_added_modified -gt 0 ]]; then
      modified="$modified$number_added_modified±"
    elif [[ $number_added_modified -gt 0 ]]; then
      modified=" ●$number_added_modified±"
    fi
    ...
    if [[ $has_diverged == false && $commits_ahead -gt 0 ]]; then to_push=" $fg_bold[black]↑$commits_ahead$fg_bold[$fgclr]"; fi
...
prompt_time() {
  prompt_segment blue cyan "$fg_bold[cyan]%D{%a %e %b - %H:%M}$fg_no_bold[cyan]"
}
